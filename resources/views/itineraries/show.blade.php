        ...
        <div class="card overflow-hidden">
            ...
            <div class="p-5 sm:p-8">
                ...
                <div class="mt-6 flex flex-wrap gap-2">
                    @if($v2)<a href="{{ route('itineraries.journal', $itinerary) }}" class="btn btn-md btn-ink"><span class="material-symbols-outlined" style="font-size:18px">auto_stories</span>{{ __('Carnet de voyage') }}</a>@endif
                    ...
                    @if($v2)<form method="POST" action="{{ route('walks.store') }}">@csrf<input type="hidden" name="itinerary_id" value="{{ $itinerary->id }}"><button class="btn btn-md btn-soft" title="{{ __('Positions des amis en direct, point de rendez-vous, messages et photos') }}"><span class="material-symbols-outlined" style="font-size:18px">groups</span>{{ __('À plusieurs') }}</button></form>@endif
    ...
    @if($v2)
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const C = window.Camino, r = @js(['start' => $result['start'], 'steps' => collect($steps)->map(fn ($s) => ['lat' => $s['lat'], 'lng' => $s['lng'], 'order' => $s['order']])->all(), 'geometry' => $result['geometry'] ?? [], 'mode' => $result['mode'] ?? 'walk', 'end' => $result['end'] ?? null]);

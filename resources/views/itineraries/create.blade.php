                    ...
                    <div class="rounded-4xl bg-ink text-white relative overflow-hidden result-pop">
                        ...
                        <div class="relative p-5 sm:p-7">
                            ...
                            <div class="mt-5 flex flex-wrap gap-2">
                                ...
                                @if(app(\App\Services\AiService::class)->enabled())<button type="button" @click="window.caminoAssistantContext = () => ({ page: 'result', title: @js($result['title'] ?? ''), steps: @js(collect($result['steps'] ?? [])->map(fn ($s) => ['title' => $s['title'], 'arrive' => $s['arrive_at'] ?? null])->values()->all()), lat: @js($result['start']['lat'] ?? null), lng: @js($result['start']['lng'] ?? null), radius: 2500, cart: @js(array_values(session('itinerary_place_ids', []))) }); $dispatch('open-assistant')" class="btn btn-lg bg-white/10 text-white border border-white/15 hover:bg-white/20" title="{{ __('Un café, l\'histoire d\'un lieu, le temps qu\'il reste : demande à CAMINO') }}"><span class="material-symbols-outlined text-sun" style="font-size:20px">auto_awesome</span>{{ __('Demander à CAMINO') }}</button>@endif

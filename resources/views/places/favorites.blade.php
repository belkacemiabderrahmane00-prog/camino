<x-app-layout title="Mes favoris">
    <section class="max-w-7xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12">
        <div class="flex flex-wrap items-end justify-between gap-4 mb-6">
            <div>
                <p class="eyebrow mb-1.5">Ma collection</p>
                <h1 class="display text-4xl">Mes favoris</h1>
            ...
            <form method="GET" action="{{ route('places.favorites') }}" class="card flex items-center gap-2 pl-4 pr-2 py-1.5 w-full sm:w-80">
                ...
                <input type="search" name="q" value="{{ $search }}" placeholder="Rechercher dans mes favoris" class="flex-1 border-0 bg-transparent focus:ring-0 text-sm">
        ...
        <div class="flex gap-2 overflow-x-auto hide-scrollbar pb-2 mb-5">
            <a href="{{ route('places.favorites', array_filter(['q' => $search])) }}" class="chip shrink-0" data-active="{{ empty($selectedCategoryId) ? 'true' : 'false' }}">Tous</a>
        ...
        @else
            <div class="card p-10 text-center">
                ...
                <p class="text-sm text-ink-muted mt-1">Explore la carte et enregistre les lieux qui te plaisent.</p>
                <a href="{{ route('map.index') }}" class="btn btn-md btn-primary mt-4">Ouvrir la carte</a>
            </div>
        @endif
    </section>
</x-app-layout>

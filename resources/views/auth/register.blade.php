<x-guest-layout title="{{ __('Créer un compte') }}">
    <p class="eyebrow mb-2">{{ __('Bienvenue') }}</p>
    <h1 class="display text-4xl">{{ __('Créer un compte') }}</h1>
    <p class="mt-2 text-sm text-ink-muted">{{ __('Gratuit. Favoris, parcours enregistrés, avis, alertes et recommandations personnalisées.') }}</p>

    <form method="POST" action="{{ route('register') }}" class="mt-8 space-y-4">
        @csrf
        <div>
            <label class="label" for="name">{{ __('Prénom ou pseudo') }}</label>
        ...
        <button type="submit" class="btn btn-lg btn-primary w-full">{{ __('Créer mon compte') }}</button>
    </form>
    <p class="mt-6 text-sm text-ink-muted text-center">{{ __('Déjà inscrit ?') }} <a href="{{ route('login') }}" class="font-semibold text-ink underline">{{ __('Connexion') }}</a></p>
</x-guest-layout>

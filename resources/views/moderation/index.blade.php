<x-app-layout title="Modération">
    <section class="max-w-7xl mx-auto px-4 sm:px-6 pt-8 sm:pt-12" x-data="{ tab: 'places' }">
        <div class="mb-6">
            <p class="eyebrow mb-1.5">Administration</p>
            <h1 class="display text-4xl">Modération</h1>
        </div>
        <div class="flex gap-2 overflow-x-auto hide-scrollbar pb-2 mb-5">
            @foreach([['places', 'Lieux proposés', 'add_location_alt'], ['photos', 'Photos', 'photo_library'], ['alerts', 'Alertes actives', 'campaign'], ['reports', 'Signalements', 'flag']] as [$k, $l, $i])
            ...
            @forelse($pendingPlaces as $place)
                <div class="card p-4 flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="flex sm:flex-col gap-2 shrink-0">
                        <form method="POST" action="{{ route('moderation.places.update', $place) }}">@csrf<input type="hidden" name="action" value="approve"><button class="btn btn-sm btn-teal w-full">Publier</button></form>
                        <form method="POST" action="{{ route('moderation.places.update', $place) }}">@csrf<input type="hidden" name="action" value="reject"><button class="btn btn-sm btn-soft w-full">Refuser</button></form>
                    </div>
                </div>
            @empty
                <div class="card p-8 text-center text-sm text-ink-muted">Aucun lieu en attente.</div>
            @endforelse
        </div>
                ...
                <div class="card overflow-hidden">
                    ...
                    <div class="p-3">
                        ...
                        <div class="flex gap-2 mt-2">
                            <form method="POST" action="{{ route('moderation.photos.update', $photo) }}" class="flex-1">@csrf<input type="hidden" name="action" value="approve"><button class="btn btn-sm btn-teal w-full">Publier</button></form>
                            <form method="POST" action="{{ route('moderation.photos.update', $photo) }}" class="flex-1">@csrf<input type="hidden" name="action" value="reject"><button class="btn btn-sm btn-soft w-full">Refuser</button></form>
                        </div>
                    </div>
                </div>
            @empty
                <div class="card p-8 text-center text-sm text-ink-muted sm:col-span-3">Aucune photo en attente.</div>
            @endforelse
        </div>
        ...
        <div x-show="tab === 'alerts'" x-cloak class="space-y-2">
            @forelse($alerts as $alert)
                <div class="card p-3 flex items-center gap-3">
                    ...
                    <form method="POST" action="{{ route('moderation.alerts.hide', $alert) }}">@csrf<button class="btn btn-sm btn-soft">Masquer</button></form>
                </div>
            @empty
                <div class="card p-8 text-center text-sm text-ink-muted">Aucune alerte active.</div>
            @endforelse
        </div>
        ...
        <div x-show="tab === 'reports'" x-cloak class="space-y-2">
            @forelse($reports as $report)
                <div class="card p-3 flex items-center gap-3">
                    ...
                    <form method="POST" action="{{ route('moderation.reports.resolve', $report) }}">@csrf @method('DELETE')<button class="btn btn-sm btn-soft">Traité</button></form>
                </div>
            @empty
                <div class="card p-8 text-center text-sm text-ink-muted">Aucun signalement.</div>
            @endforelse
        </div>
    </section>
</x-app-layout>

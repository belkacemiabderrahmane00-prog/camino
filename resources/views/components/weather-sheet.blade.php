        ...
        <div x-show="open" x-transition:enter="animate-sheet-up" class="relative w-full sm:max-w-md card rounded-b-none sm:rounded-3xl overflow-hidden max-h-[88vh] overflow-y-auto">
            <div class="bg-gradient-to-br {{ $tones[$wa['tone'] ?? 'neutral'] }} text-white p-5 sm:p-6">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-[0.16em] opacity-90">{{ __('Paris · maintenant') }}</p>
                        ...
                        <p class="mt-1 text-sm opacity-90">{{ __($wf['current']['label'] ?? 'Météo indisponible') }}@if($wf['current'] && $wf['current']['wind']) · {{ __('vent') }} {{ round($wf['current']['wind']) }} km/h @endif</p>
                    </div>
                    <button @click="open = false" class="h-9 w-9 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30" aria-label="{{ __('Fermer') }}"><span class="material-symbols-outlined" style="font-size:18px">close</span></button>
                </div>
                @if($wa)
            ...
            <div class="p-5 sm:p-6 space-y-5">
                @if($nextHours->isNotEmpty())
                    <div>
                        <p class="label">{{ __('Prochaines heures') }}</p>
                ...
                @if(!empty($wf['days']))
                    <div>
                        <p class="label">{{ __('Trois jours') }}</p>
                        <div class="space-y-1.5">
                            @foreach(array_slice($wf['days'], 0, 3) as $i => $day)
                                <div class="flex items-center gap-3 rounded-2xl bg-paper px-3 py-2 text-sm">
                                    ...
                                    <span class="text-ink-muted flex-1 truncate">{{ __($day['label']) }}</span>
                ...
                @endif
                <a href="{{ route('itineraries.create') }}" class="btn btn-md btn-primary w-full"><span class="material-symbols-outlined" style="font-size:18px">auto_awesome</span>{{ __('Générer un parcours adapté à la météo') }}</a>
                <p class="text-[10px] text-ink-muted text-center">{{ __('Données Open-Meteo et MET Norway, mises à jour toutes les 30 minutes.') }}</p>
            </div>
        </div>
    </div>
</div>

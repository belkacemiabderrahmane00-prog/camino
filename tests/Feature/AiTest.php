...
class AiTest extends TestCase
{
    ...
    public function test_a_retired_model_is_replaced_automatically(): void
    {
        ...
        $ai = app(AiService::class);
        $this->assertSame('Nouveau Gemini', $ai->chat('sys', [['role' => 'user', 'content' => 'salut']]));
        $this->assertSame('gemini-4.1-flash', Cache::get('ai_model_gemini'));

        config(['camino.ai.gemini_key' => '']);
        $this->assertSame('Nouveau Groq', $ai->chat('sys', [['role' => 'user', 'content' => 'salut']]));
        $this->assertSame('meta-llama/llama-4-maverick-17b-128e-instruct', Cache::get('ai_model_groq'));
        $this->assertSame('meta-llama/llama-4-maverick-17b-128e-instruct', $ai->providers()[0]['model']);
    }
}
